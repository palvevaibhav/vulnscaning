...
func (p *Publisher) SendSbomToConsole(sbom []byte, skipScan bool) error {
	...
	pdata.SetScanType("VulnerabilityScan")
	pdata.SetScanId(p.config.ScanID)
	pdata.SetProgressStatus("stage2")
	...
	log.Debugf("publish scan progress status response: %v", pResp)
