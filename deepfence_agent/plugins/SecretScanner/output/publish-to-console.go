...
func IngestSecretScanResults(secretScanMsg string, index string) error {
	log.Debugf("Ingesting secret scan results: %s\n", secretScanMsg)
...
func (p *Publisher) IngestScanProgressStatus(ctx context.Context, scanID, scanType string, status string) error {
	log.Debugf("Ingesting scan progress status for scanID: %s, scanType: %s, status: %s\n", scanID, scanType, status)
	// Set progress status
	pdata := dsc.UtilsScanProgressStatusRequest{}
	pdata.SetScanType(scanType)
	pdata.SetScanId(scanID)
	pdata.SetProgressStatus(status)

	// Ingest scan progress status
	pReq := p.client.Client().VulnerabilityAPI.IngestScanProgressStatus(ctx)
	pReq = pReq.UtilsScanProgressStatusRequest(pdata)

	pResp, err := p.client.Client().VulnerabilityAPI.IngestScanProgressStatusExecute(pReq)
	if err != nil {
		return err
	}
	log.Debugf("Ingest secret scan progress status response: %v", pResp)
	return nil
}
...
func (p *Publisher) IngestSecretScanResults(scanID string, secrets []SecretFound) error {
	log.Debugf("Ingesting secret scan results for scanID: %s\n", scanID)
