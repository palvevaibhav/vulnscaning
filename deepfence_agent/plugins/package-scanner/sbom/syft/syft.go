...
//   "/mnt", "/run", "/proc", "/dev", "/boot", "/sys", "/lost+found", "/data1", "/data2"
//  }
...
var (
    linuxExcludeDirs = []string{
        "/var/lib/docker", "/var/lib/containerd", "/var/lib/containers",
        "/var/lib/crio", "/var/run/containers", "/home/kubernetes/containerized_mounter",
        "/mnt", "/run", "/proc", "/dev", "/boot", "/sys", "/lost+found", "/data*",
        "/tmp", "/tmp_backup", "/var/log*", "/var/cache", "/var/backup*", "/var/spool", "/archives", "/media",
        "/var/lib/docker", 
      //   
      //   "/home", "/srv", "/root",
      //   "/bin", "/usr", "/lib", "/lib64", "/etc", "/sbin", "/opt",
    }
    mntDirs = getNfsMountsDirs()
)
...
//  log.Debugf("syftArgs: %+v", syftArgs)
...
func buildCatalogersArg(scanType string, isRegistry bool) []string {
 ...
 //syftArgs = append(syftArgs, "--catalogers", "rpm-db-cataloger")
