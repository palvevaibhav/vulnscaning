...
func (c ExtendedContainerd) GetFileSystemPath(containerId string, namespace string) ([]byte, error) {
	...
	logrus.Infof("GetFileSystemPath containerId: %v, namespace: %v\n", containerId, namespace)

	client, err := containerdApi.New(strings.Replace(c.GetSocket(), "unix://", "", 1))
	if err != nil {
		logrus.Errorf("containerApi ClientError: %s", err.Error())
		return []byte(""), err
	}
	defer client.Close()
	...
	logrus.Infof("GetFileSystemPath container: %v\n", container)
	info, _ := container.Info(ctx)
	logrus.Infof("GetFileSystemPath Container Info: %+v\n", info)
...
var _ vesselent.EntRuntime = (*ExtendedContainerd)(nil)
