...
func PopulateFinalReport(vulnerabilities []byte, cfg utils.Config) ([]scanner.VulnerabilityScanReport, error) {
	...
	//	}
		...
		var cveFixedIn string
		if len(match.Vulnerability.Fix.Versions) != 0 {
			cveFixedIn = match.Vulnerability.Fix.Versions[0]
		}
		...
		metasploitURL, urls := utils.ExtractExploitPocURL(match.Vulnerability.URLs)
                var convertedLocations []scanner.ScannerCoordinates
		for _, loc := range match.Artifact.Locations {
			convertedLocations = append(convertedLocations, scanner.ScannerCoordinates{
		...
		if len(match.MatchDetails) > 0 {
			packageName = match.MatchDetails[0].SearchedBy.Package.Name
		}	
		...
		report := scanner.VulnerabilityScanReport{
			...
			ParsedAttackVector: "",
			Identifier:         fmt.Sprintf("%s-%s-%s", match.Vulnerability.ID, match.Vulnerability.Namespace, packageName),
			FixState:           match.Vulnerability.Fix.State,
			FixedInVersion:     cveFixedIn,
		}
		if len(convertedLocations) > 0 {
 		   report.CveCausedByPackagePath = combinePaths(convertedLocations)
		}else {
...
func combinePaths(paths []scanner.ScannerCoordinates) string {
