...
func (s *gRPCServer) FindMalwareInfo(c context.Context, r *pb.MalwareRequest) (*pb.MalwareResult, error) {
	go func() {
		...
		go func() {
			writeSecretScanProgressStatus(r.GetScanId(), "MalwareScan", "stage2")
		}()

		fmt.Println("In-progress secret scan for request: stage3")
		var prevMalware output.IOCFound
		var hasPrev bool

		for malware := range malwares {
			if hasPrev {
				writeScanDataCommon(malware, r.GetScanId(), trim, isContainer, "")
			}
			prevMalware = malware
			hasPrev = true
		}
		...
		/**
		if hasPrev {
			writeScanDataCommon(prevMalware, r.GetScanId(), trim, isContainer, "stage4")
		}
		*/
		...
		go func() {
			if hasPrev {
				writeSecretScanProgressStatus(r.GetScanId(), "MalwareScan", "stage3")
				writeScanDataCommon(prevMalware, r.GetScanId(), trim, isContainer, "stage4")
			} else {
				fmt.Println("Completed secret scan for request: stage4")
				writeSecretScanProgressStatus(r.GetScanId(), "MalwareScan", "stage4")
			}
		}()
...
func writeScanDataCommon(malware  output.IOCFound, scan_id string, trim bool, isContainer bool, progressStatus string) {
	if isContainer {
	...
	output.WriteScanData([]*pb.MalwareInfo{output.MalwaresToMalwareInfo(malware)}, scan_id, progressStatus)
}
...
func writeSecretScanProgressStatus(scanId string, scanType string, status string) {
	consoleUrl := os.Getenv("MGMT_CONSOLE_URL")
	consolePort := os.Getenv("MGMT_CONSOLE_PORT")
	consoleKey := os.Getenv("DEEPFENCE_KEY")
	fmt.Printf("console_url: %v, port: %v, key: %v", consoleUrl, consolePort, consoleKey)
	if consoleUrl != "" && consolePort != "" && consoleKey != "" {
		pub, err := output.NewPublisher(consoleUrl, consolePort, consoleKey)
		if err != nil {
			log.Error(err.Error())
		}
