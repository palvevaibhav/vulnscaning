...
func (c *ComplianceScanner) RunComplianceScan() error {
	...
	logrus.Info("RunComplianceScan: stage2")
	if err != nil {
		return err
	}
	...
	scriptConfig, err := LoadConfig()
	if err != nil {
		logrus.Errorf("Error loading script config: %v", err)
		return err
	}
	...
	logrus.Infof("ComplianceCheckTypes:%+v", c.config.ComplianceCheckTypes)
	...
	stopped := false
	for i := range c.config.ComplianceCheckTypes {
		complianceCheckType := c.config.ComplianceCheckTypes[i]
	// for _, complianceCheckType := range c.config.ComplianceCheckTypes {
		...
		logrus.Infof("Compliance scan results for type %s: %d items", complianceCheckType, len(benchItems))
		...
		logrus.Infof("Compliance scan results for type %s: %d compliance docs", complianceCheckType, len(complianceScanResults))
		err = c.IngestComplianceResults(complianceScanResults)
		if err != nil {
			return err
		}
		if i == len(c.config.ComplianceCheckTypes) - 1 {
			logrus.Info("RunComplianceScan will send last stage AS: stage4")
		}
	}
	...
	logrus.Info("RunComplianceScan: stage3")
